...
Populations in the real world are heterogeneous! And treatments (medical, economic, political, etc.) affect those different populations differently. If we want to ensure the best outcomes for everyone, we need to estimate these effects


**Q2. What are some challenges with estimating this effect?**
The main challenge with estimating this effect is that for each participant, we only ever observe either $Z=1$ or $Z=0$. So we can never directly estimate CATE from data.
...
propensities = c(mean(Z[X==1]),mean(Z[X==2]),mean(Z[X==3]))
t1 = calculate_tau(X[X==1],Y[X==1],Z[X==1],propensities[1])
t2 = calculate_tau(X[X==2],Y[X==2],Z[X==2],propensities[2])
t3 = calculate_tau(X[X==3],Y[X==3],Z[X==3],propensities[3])
...
ps.model = glm(Z~X,family = binomial(link = "logit"))
ps.est = predict.glm(ps.model,as.data.frame(X),type = "response")
```


```{r Fitting and visualizing causal forest}
#causal forest
...
```{r Fitting BART, modeling tau(X) directly }
...
### and the splitting probability discussed earlier, respectively. How does changing these affect your results? what about the number of trees? Have students try this out with their neighbors
...
```{r Fitting BART, separate models for mu}
...
```{r Fitting Bayesian causal forest }
